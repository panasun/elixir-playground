defmodule Graphql.OrderingTest do
  use Graphql.DataCase

  alias Graphql.Ordering

  describe "orders" do
    alias Graphql.Ordering.Order

    import Graphql.OrderingFixtures
    ...
    test "create_order/1 with valid data creates a order" do
      valid_attrs = %{customer_number: 42, items: %{}, ordered_at: ~U[2023-05-28 04:11:00Z], state: "some state"}

      assert {:ok, %Order{} = order} = Ordering.create_order(valid_attrs)
      assert order.customer_number == 42
      assert order.items == %{}
      assert order.ordered_at == ~U[2023-05-28 04:11:00Z]
    ...
    test "update_order/2 with valid data updates the order" do
      order = order_fixture()
      update_attrs = %{customer_number: 43, items: %{}, ordered_at: ~U[2023-05-29 04:11:00Z], state: "some updated state"}

      assert {:ok, %Order{} = order} = Ordering.update_order(order, update_attrs)
      assert order.customer_number == 43
      assert order.items == %{}
      assert order.ordered_at == ~U[2023-05-29 04:11:00Z]
